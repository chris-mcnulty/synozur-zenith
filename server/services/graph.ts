...
export interface SiteDocumentLibrary {
  ...
  createdGraphAt: string;
}

export async function fetchSiteDocumentLibraries(token: string, graphSiteId: string): Promise<SiteDocumentLibrary[]> {
  try {
    // 1. Fetch document libraries (lists)
    const listUrl = `https://graph.microsoft.com/v1.0/sites/${graphSiteId}/lists?$filter=list/template eq 'documentLibrary'&$select=id,displayName,description,webUrl,list,lastModifiedDateTime,createdDateTime`;
    ...
    if (!listRes.ok) {
      ...
      console.warn(`[graph-service] Error fetching lists for ${graphSiteId}: ${errText}`);
      return [];
    }
    ...
    // 2. Fetch drives to get storage and item counts
    const driveUrl = `https://graph.microsoft.com/v1.0/sites/${graphSiteId}/drives?$select=id,quota,system,name,driveType,webUrl`;
    ...
    if (driveRes.ok) {
      const driveData = await driveRes.json();
      drives = driveData.value || [];
    }

    // 3. Map lists to libraries and enrich with drive data
    return lists.map((list: any) => {
      const drive = drives.find(d => d.webUrl === list.webUrl || d.name === list.displayName);
      ...
      return {
        ...
        webUrl: list.webUrl,
        template: list.list?.template || 'documentLibrary',
        itemCount: 0, // Placeholder
        storageUsedBytes: drive?.quota?.used || 0,
        sensitivityLabelId: list.list?.sensitivityLabel?.id || undefined,
        isDefaultDocLib: list.displayName === 'Documents' || (drive?.driveType === 'documentLibrary' && list.displayName === 'Shared Documents'),
        hidden: list.list?.hidden || false,
        lastModifiedAt: list.lastModifiedDateTime,
        createdGraphAt: list.createdDateTime,
      };
    });
  } catch (err: any) {
    console.error(`[graph-service] Failed to fetch libraries: ${err.message}`);
    return [];
  }
}
