...
- **Nightly Data Refresh & Health Report (premium / Enterprise)**: A nightly scheduler (`server/services/nightly-health-report-scheduler.ts`, registered in `server/index.ts`) gated on the new `scheduledHealthReports` plan feature (Enterprise only). For each active tenant connection it (1) refreshes workspaces (sites) and document libraries via `runSharePointTenantSync` so storage/quota figures are current, then (2) generates a Nightly Health Report (`server/services/nightly-health-report.ts`) flagging sites over 75% and over 90% of storage quota plus other governance health issues (sites/libraries missing sensitivity labels, single-owner/orphaned sites, external sharing, 90-day stale sites, degraded tenant connection). Reports persist to the new `nightly_health_reports` table and are delivered **in-product** (page at `/app/nightly-health-report`; in-app notifications via `createNotification`, category `governance_report`) and **by email** (`sendNightlyHealthReportEmail` in `server/email-support.ts`, SendGrid) to Tenant Admins, Governance Admins, and Platform Owners. Three independent per-connection admin toggles on `tenant_connections` (`nightly_refresh_schedule_enabled`, `nightly_health_report_enabled`, `nightly_health_report_email_enabled`, all default true) let admins turn report generation and/or email delivery off while still running the nightly data refresh. Each scheduled pass writes a `scheduled_job_runs` row of type `nightlyHealthReport`. Routes (`server/routes/nightly-health-report.ts`, all `requireFeature("scheduledHealthReports")`): `GET/PATCH /api/nightly-health-reports/settings`, `POST /api/nightly-health-reports` (on-demand, 202 + poll), `GET /api/nightly-health-reports[?tenantConnectionId]`, `GET /api/nightly-health-reports/:id`, `DELETE /api/nightly-health-reports/:id`. Migration `0024_nightly_health_reports.sql`; schema mirrored in `ensureTenantConnectionsSchema()`.
