...
- **Comprehensive Audit Trail (BL-001)**: Every privileged mutation writes to `audit_log` (auth, tenant lifecycle, workspace governance, policy CRUD, custom fields, data dictionary, label sync, MSP grants, org/admin changes, access denials, sync runs). Centralized helper at `server/services/audit-logger.ts` (typed `AUDIT_ACTIONS` enum + `logAuditEvent` wrapper). RBAC `requireRole` / `requirePermission` / `requireAnyPermission` all log `ACCESS_DENIED` on rejection. Background sync jobs auto-emit `SYNC_STARTED`/`SYNC_COMPLETED`/`SYNC_FAILED` via `trackJobRun`. Append-only at the API layer (no DELETE/UPDATE endpoint exists). Admin viewer at `/app/admin/audit-log` (Platform Owner, Tenant Admin, Read-Only Auditor) with action/user/tenant/date filters and CSV/JSON export. Retention enforced per service plan via daily scheduler (TRIAL 30d, STANDARD 365d, PROFESSIONAL 7y, ENTERPRISE unlimited).
