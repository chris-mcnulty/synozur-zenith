...
- **Copilot Prompt Intelligence (BL-038)**: Syncs Copilot interaction history via `/beta` Graph endpoint with plain `fetch()` (not `graphFetchWithRetry`). No client-side watermark/date filtering at fetch level — ALL items returned by the API are collected; retention (30-day) filter applied at upsert time; DB unique constraint handles duplicates. 429 rate limits retried with Retry-After back-off (up to 3 retries per user). UPN fallback on first-page 404 (one attempt). Mid-stream 404/403 keeps partial items without advancing watermark. Inter-page delay 500ms, inter-user delay 2s. User list from `license_assignments` filtered by Copilot SKU, NO `accountEnabled` filter. UX shows sync status with polling, interaction count, and completion banners.
